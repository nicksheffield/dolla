...
function dolla(selector, context){
	...
	d.on = function(event, selector, callback){
		...
		d.each(function(element){
			events.forEach(function(thisEvent){
				element.addEventListener(thisEvent, function(event){
					var res;
					if(typeof selector == 'string'){
						if(matches(event.target, selector)){
							res = callback.call(event.target, event);
						}
					}else{
						res = callback.call(element, event);
					}
	...
	function become(element){
		...
		if(typeof element.length == 'number' && element != window){
